...
{
	static TypeId tid =
			TypeId("ns3::ndn::Consumer").SetGroupName("Ndn").SetParent<App>().AddAttribute(
					"StartSeq", "Initial sequence number", IntegerValue(0),
					MakeIntegerAccessor(&Consumer::m_seq),
					MakeIntegerChecker<int32_t>())

			.AddAttribute("Prefix", "Name of the Interest", StringValue("/"),
					MakeNameComponentsAccessor(&Consumer::m_interestName),
					MakeNameComponentsChecker()).AddAttribute("LifeTime",
					"LifeTime for interest packet", StringValue("2s"),
			...
			.AddAttribute("RetxTimer",
					...
					MakeTimeAccessor(&Consumer::GetRetxTimer,
							&Consumer::SetRetxTimer), MakeTimeChecker())
			...
			.AddTraceSource("FirstInterestDataDelay",
					...
					MakeTraceSourceAccessor(
							&Consumer::m_firstInterestDataDelay));

	return tid;
}

Consumer::Consumer() :
		m_rand(0, std::numeric_limits<uint32_t>::max()), m_seq(0), m_seqMax(0) // don't request anything
{
...
void Consumer::SendPacket()
{
	...
	//
	Ptr<NameComponents> nameWithSequence = Create<NameComponents>(
			m_interestName);
...
void Consumer::OnContentObject(const Ptr<const ContentObjectHeader> &contentObject,	Ptr<Packet> payload)
{
	if (!m_active)
		return;
	...
	uint32_t seq = boost::lexical_cast<uint32_t>(
			contentObject->GetName().GetComponents().back());

	NS_LOG_INFO ("< DATA for " << seq);

	if (contentObject->IsCorrupted())
	{
		NS_LOG_INFO ("################## É Fake #########################\n");
	}
	else
...
void Consumer::OnNack(const Ptr<const InterestHeader> &interest,
		Ptr<Packet> origPacket)
{
	...
	uint32_t seq = boost::lexical_cast<uint32_t>(
			interest->GetName().GetComponents().back());
